...
const CatalogYonetimi: React.FC = () => {
  ...
  // 1. API endpoint'i `/catalogs` olarak düzeltildi.
  ...
  const handleFormSubmit = useCallback(async (values: any) => {
    try {
      ...
      if (editId) {
        await api.put(`/catalogs/${editId}`, payload);
      } else {
      ...
      Swal.fire({
        title: 'Başarılı!',
        text: editId ? 'Katalog başarıyla güncellendi.' : 'Katalog başarıyla eklendi.',
        icon: 'success',
  ...
  const handleDelete = useCallback(async (id: string) => {
    try {
      ...
      Swal.fire('Silindi!', 'Katalog başarıyla silindi.', 'success');
  ...
  const handleToggleActive = useCallback(async (id: string, newState: boolean) => {
    ...
    try {
      const confirmed = await Swal.fire({
        title: `Kataloğu ${newState ? 'aktif' : 'pasif'} yapmak istediğinize emin misiniz?`,
        icon: 'question',
        showCancelButton: true,
        confirmButtonText: 'Evet',
        cancelButtonText: 'Hayır',
      ...
      await api.put(`/catalogs/${id}`, payload);
      await fetchSections(); // Listeyi yenile
      Swal.fire('Başarılı!', `Katalog durumu başarıyla güncellendi.`, 'success');
...
export default CatalogYonetimi;
