...
const BrandRow = React.memo(({ section, onToggle, onEdit, onDelete, onOpenTextModal, onOpenImageModal }: {
  section: Section;
...
const BrandYonetimi: React.FC = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      const result = await Swal.fire({
        title: editId ? 'Marka güncellensin mi?' : 'Yeni Marka eklensin mi?',
      ...
      if (result.isConfirmed) {
        ...
        try {
          ...
          Swal.fire({
            title: 'Başarılı!',
            text: editId ? 'Marka güncellendi.' : 'Marka eklendi.',
            icon: 'success',
  ...
  const handleDelete = useCallback(async (id: string) => {
    const result = await Swal.fire({
      title: 'Marka silinsin mi?', text: 'Bu işlem geri alınamaz!',
      icon: 'warning', showCancelButton: true, confirmButtonText: 'Evet', cancelButtonText: 'Hayır',
      buttonsStyling: false,
      customClass: {
        actions: 'flex justify-center gap-4',
    ...
    if (result.isConfirmed) {
      ...
      try {
        ...
        Swal.fire({
          title: 'Silindi!', text: 'Marka başarıyla silindi.', icon: 'success', showConfirmButton: true,
  ...
  const toggleActiveStatus = useCallback(async (section: Section) => {
    ...
    if (result.isConfirmed) {
      ...
      try {
        ...
        await api.put(`/brands/${section.id}`, payload);
        Swal.fire({
          title: 'Başarılı!', text: 'Marka durumu güncellendi.', icon: 'success', showConfirmButton: true,
          customClass: { actions: 'flex justify-center gap-4', confirmButton: 'bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700' },
        });
        fetchSections();
      } catch (err: any) {
          ...
          <table className="min-w-full table-auto border-collapse text-sm">
            ...
            <tbody>
              {sections.map((section) => (
                <BrandRow key={section.id} section={section} onEdit={handleEdit} onDelete={handleDelete}
